( function ( apiFetch, components, element, i18n ) {
	...
	const utils = window.nervCoreControlUtils || {};
	const statusClass = utils.statusClass;
	const metricCard = utils.metricCard;
	const healthRow = utils.healthRow;
	const tabButton = utils.tabButton;
	const stepItem = utils.stepItem;
	const activityRows = utils.activityRows;
	const aiProviderDefaults = utils.aiProviderDefaults;
	const cloneAiFeature = utils.cloneAiFeature;
	const cloneAiServicesForm = utils.cloneAiServicesForm;
	const cloneBrandForm = utils.cloneBrandForm;
	const cloneSeoForm = utils.cloneSeoForm;
	const clonePanelsForm = utils.clonePanelsForm;
	const normalizePanelOrder = utils.normalizePanelOrder;
	const normalizeCustomPanelOrder = utils.normalizeCustomPanelOrder;
	const reorderPanels = utils.reorderPanels;
	const cloneGeoForm = utils.cloneGeoForm;
	const cloneEffectsForm = utils.cloneEffectsForm;
	const cloneAppearanceForm = utils.cloneAppearanceForm;
	const effectsSettingsPayload = utils.effectsSettingsPayload;
	const effectsPresetDocument = utils.effectsPresetDocument;
	const mergeEffectsPresetDocument = utils.mergeEffectsPresetDocument;
	const cloneArticlesForm = utils.cloneArticlesForm;
	const cloneMobileForm = utils.cloneMobileForm;
	const cloneSocialForm = utils.cloneSocialForm;
	const clonePartnersForm = utils.clonePartnersForm;
	const cloneToolsForm = utils.cloneToolsForm;
